...
ftrs <- as.character(read.table("UCI HAR Dataset/features.txt")[[2]])

# load and merge the test data
testdt <- read.table("UCI HAR Dataset/test/X_test.txt", col.names = ftrs)
...
trndt <- read.table("UCI HAR Dataset/train/X_train.txt", col.names = ftrs)
trnsub <- read.table("UCI HAR Dataset/train/subject_train.txt", col.names = "subjectid")
trnact <- read.table("UCI HAR Dataset/train/y_train.txt", col.names = "activity")
...
levels(alldata$activity) <- read.table("UCI HAR Dataset/activity_labels.txt")[[2]]
